...
void Triangle::Draw(const Transform& transform, const Vector4& color) {
	...
	Matrix4x4 projectionMatrixSprite = Matrix4x4::MakeOrthographicMatrix(0.0f, 0.0f, static_cast<float_t>(windowsSetup_->GetClientSize().x), static_cast<float_t>(windowsSetup_->GetClientSize().y), 0.0f, 100.0f);
