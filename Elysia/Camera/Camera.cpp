...
void Camera::Initialize() {
	...
	//アスペクト比
	aspectRatio = static_cast<float_t>(Elysia::WindowsSetup::GetInstance()->GetClientSize().x) / static_cast<float_t>(Elysia::WindowsSetup::GetInstance()->GetClientSize().y);
	...
	//正射影行列(正規化)を計算
	orthographicMatrix = Matrix4x4::MakeOrthographicMatrix(0.0f, 0.0f, static_cast<float_t>(Elysia::WindowsSetup::GetInstance()->GetClientSize().x), static_cast<float_t>(Elysia::WindowsSetup::GetInstance()->GetClientSize().y), 0.0f, 100.0f);
}
