...
void Camera::Initialize() {
	//リソース作成
	resource = Kamaboko::DirectXSetup::GetInstance()->CreateBufferResource(sizeof(CameraMatrixData)).Get();

	//アスペクト比
	aspectRatio = static_cast<float_t>(Kamaboko::WindowsSetup::GetInstance()->GetClientWidth()) / static_cast<float_t>(Kamaboko::WindowsSetup::GetInstance()->GetClientHeight());
	...
	//正射影行列(正規化)を計算
	orthographicMatrix = Matrix4x4Calculation::MakeOrthographicMatrix(0.0f, 0.0f, static_cast<float_t>(Kamaboko::WindowsSetup::GetInstance()->GetClientWidth()), static_cast<float_t>(Kamaboko::WindowsSetup::GetInstance()->GetClientHeight()), 0.0f, 100.0f);
}
