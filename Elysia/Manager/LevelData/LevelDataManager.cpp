...
void Elysia::LevelDataManager::Place(nlohmann::json& objects, LevelData& levelData) {
	...
	for (nlohmann::json& object : objects) {
		...
		if (type.compare("MESH") == 0) {
			//要素追加
			levelData.objectData.emplace_back(ObjectData{});
			//今追加した要素の参照を得る
			ObjectData& objectData = levelData.objectData.back();
...
void Elysia::LevelDataManager::Generate(LevelData& levelData) {
	...
	for (ObjectData& objectData : levelData.objectData) {
...
uint32_t Elysia::LevelDataManager::Load(const std::string& filePath) {
	...
	//一度読み込んだものはその値を返す
	//新規は勿論読み込みをする
	auto it = levelData_.find(filePath);
	if (it != levelData_.end()) {
	...
	//それぞれに情報を記録
	levelData_[fullFilePath]->folderName = folderName;
	levelData_[fullFilePath]->fileName = fileName;
...
void Elysia::LevelDataManager::Reload(const uint32_t& levelDataHandle) {
	...
	//一度全てのオブジェクトのデータを消す
	for (auto& [key, levelDataPtr] : levelData_) {
		if (levelDataPtr->handle == levelDataHandle) {

			//モデルを消す
			for (auto& object : levelDataPtr->objectData) {
				// Modelの解放
	...
	for (std::map<std::string, std::unique_ptr<LevelData>>::iterator it = levelData_.begin(); it != levelData_.end(); ++it) {
